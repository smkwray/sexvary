...
def _default_raw_dir(root: Path) -> Path:
    data_dir = first_existing_external_data_dir("psid_cds_tas", root)
    if data_dir is None:
        searched = ", ".join(str(path) for path in external_data_dirs("psid_cds_tas", root))
        raise FileNotFoundError(
            "No PSID CDS/TAS raw directory found. "
            f"Searched: {searched}. Place the public CDS/TAS zip files there or pass --raw-dir."
        )
    zip_files = sorted(path for path in data_dir.glob("*.zip") if path.is_file())
    if not zip_files:
        raise FileNotFoundError(
            f"No PSID zip files found under {data_dir}. "
            "Expected archives such as CDS2014.zip, CDS2019.zip, or CDS2021.zip."
        )
    return data_dir
...
def _build_robustness_variants(*, baseline: pd.DataFrame, registry, config) -> dict[str, pd.DataFrame]:
    ...
    nonprimary_mask = baseline.get("weight_source", pd.Series(pd.NA, index=baseline.index)).astype("string") != baseline.get(
        "weight_primary_source", pd.Series(pd.NA, index=baseline.index)
    ).astype("string")
    if nonprimary_mask.fillna(False).any():
        no_fallback = baseline.loc[~nonprimary_mask.fillna(False)].copy()
        variants["no_weight_fallback_rows"] = _estimate_from_prepared_frame(
            prepare_analysis_frame(no_fallback, config=config),
            config=config,
            registry=registry,
        )
    return variants
...
def _build_wave_status_summary(estimates: pd.DataFrame) -> pd.DataFrame:
    return (
        estimates.groupby(["cycle_or_wave", "evidence_status"], dropna=False)
        .size()
        .reset_index(name="cells")
        .sort_values(["cycle_or_wave", "cells", "evidence_status"], ascending=[True, False, True], kind="stable")
    )


def _write_psid_report(
    *,
    out_dir: Path,
    estimates: pd.DataFrame,
    robustness_summary: pd.DataFrame,
) -> Path:
    report_path = out_dir / "psid_cds_tas_report.md"
    wave_status = _build_wave_status_summary(estimates)
    cds = estimates[estimates["cycle_or_wave"].astype("string").str.startswith("CDS", na=False)]
    tas = estimates[estimates["cycle_or_wave"].astype("string").str.startswith("TAS", na=False)]
    lines = [
        "# PSID CDS / TAS report",
        "",
        "This PSID pass is supporting evidence, not headline evidence. All current inferential rows are still method-limited or provisional.",
        "",
        "## Wave-status summary",
        "",
        markdown_table(wave_status),
        "",
        "## Coverage split",
        "",
        f"- CDS rows: `{len(cds)}`",
        f"- TAS rows: `{len(tas)}`",
        f"- Provisional TAS rows: `{int((tas['evidence_status'] == 'provisional').sum())}`",
        "",
        "## Robustness checks",
        "",
        "These compare the weighted baseline against unweighted, winsorized, and fallback-free variants where applicable.",
        "",
        markdown_table(robustness_summary) if not robustness_summary.empty else "_No robustness summary available._",
        "",
        "## Interpretation guardrails",
        "",
        "- Later TAS waves are useful supporting evidence, but still rely on simple-design uncertainty.",
        "- TAS 2019 rows that depend on alternate public weights remain provisional by design.",
        "- Early TA/TAS waves are still excluded because the public sex fields are not harmonized cleanly enough for paper-ready use.",
        "",
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PSID CDS/TAS ingest -> estimate pipeline.")
    parser.add_argument("--raw-dir", help="Path to a directory containing PSID CDS/TAS public zip files.")
    parser.add_argument("--trait", action="append", help="Optional trait filter.")
    parser.add_argument("--output-dir", default="results/psid_cds_tas", help="Output directory relative to the project root.")
    ...
    adapter = PSIDAdapter(spec, raw_path=raw_dir)
    ...
    estimates_path = write_table(estimates, out_dir / "psid_cds_tas_trait_estimates.csv")
    summary = estimates[
        [
            "cycle_or_wave",
            "country",
    ...
    summary_path = out_dir / "psid_cds_tas_trait_estimates.md"
    write_markdown_summary(summary, summary_path, title="PSID CDS/TAS trait estimates")

    robustness_comparison_path = out_dir / "psid_cds_tas_robustness_comparison.csv"
    robustness_summary_path = out_dir / "psid_cds_tas_robustness_summary.csv"
    ...
    write_markdown_summary(
        robustness_summary,
        out_dir / "psid_cds_tas_robustness_summary.md",
        title="PSID CDS/TAS robustness summary",
    )
    report_path = _write_psid_report(out_dir=out_dir, estimates=estimates, robustness_summary=robustness_summary)
